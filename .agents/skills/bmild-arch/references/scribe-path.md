...
> Shared definition for all standard personas (`bmild-{pm,ux,arch,planner,dev,qa,sec}`). Each standard persona ships an identical copy under its own `references/scribe-path.md`. The active persona loads **this skill's** copy via a relative path — never `docs/`, never another skill's tree. There is no per-skill bespoke scribe logic.
...
2. **Load** the target owner's **entire `SOUL.md`** — sibling of their `SKILL.md`, resolved relative to their own skill dir (the same voice-resolution contract each persona already uses for its own `SOUL.md`, applied to the *target* owner — e.g. the UX persona skill's `SOUL.md`). Whole-file load — never a paraphrase, never a heading-boundary scrape. One voice-resolution contract, two consumers (this + roundtable attendee-voice loading).
3. **Settlement-verify** — one pass against the loaded `SOUL.md` fields (`## What I believe`, `## My vocabulary`, `## My tensions`, `## What gets under my skin`, `## My center of gravity`): *"From [owner]'s beliefs, tensions, and vocabulary, does this settled fact — and its exact wording — land correctly?"* The structured fields make this check discriminating: a "settled" wording that trips the owner's own stated *tensions* or *pet peeves* is caught here and routed.

...
- **Structural dimension sweep at finalize.** Before finalizing, walk every applicable structural dimension — boundaries and ownership, data, integration/API, service, failure and consistency, quality, technology stack, security and trust, deployment and environments, infrastructure provider, capacity, operations and observability, compatibility and evolution — and mark each decided (link its section), deferred with a reason, or an explicit open question. The operational envelope is the dimension most often left silent; a silently absent applicable dimension is a finding, while a dimension that genuinely does not apply is recorded as not applicable with its reason. The sweep is a coverage check, not a demand to commit architecture where nothing applies.
...
- [ ] Structural dimension sweep recorded: every applicable dimension decided, deferred with a reason, or an explicit open question — operational and environment dimensions included

...
2. **Hydrate PM inputs** — Read current `product-brief.md` and `prd.md` before asking refinement questions. Treat explicit PM requirements as settled unless the refinement target exposes a conflict, stale requirement, implementation impossibility, or architecture-owned trade-off.

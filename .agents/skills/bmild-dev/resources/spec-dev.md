...
- **Readiness by meaning.** Establish intent, observable acceptance, usable constraints, and relevant proof obligations using `.agents/skills/bmild-planner/resources/readiness-verification.md` as the canonical criteria. No particular combination of brief/PRD/UX/architecture filenames is an eligibility gate. Consult only for an unresolved consequential gap. Continue independent authorized work when another part is blocked.

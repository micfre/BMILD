...
4. **[x] Proceed** — Close the elicitation session:
   ...
   - **Ratification→Promotion gate.** When the session ends with a user-ratified durable-contract change, load `references/promotion-protocol.md`. Inventory, ask once, apply only explicitly authorized mechanical lines, return independent owner episodes through the standard ladder, and offer Course-Correction once for coupled fallout. Without facilitator-write authorization, return the inventory to the convener. Name the close state when the gate fires: `ratified_and_promoted` | `ratified_and_routed` | `ratified_pending_authorization` | `ratified_with_documentation_deferred`.
   - Sign off with a **branch-aware close** — never *"I will turn this back to [persona]"* for work you are not continuing in this turn. Use the convener identity and suspended session state carried from step-01:
     - **Persona convened (default; resume in-turn)** — when the suspended session state was recorded at open and the convener's session is present in this conversation, resume it yourself: *"Facilitator ⚡ closing. Refined content is ready. [If gate fired: Close state: [state].] Resuming **[convener name] [icon]** at [artifact section in progress] with the refined content as input."* — then continue this same turn as the convener per its Same-Session Resumption contract: no Opening Stance re-emit, resume the suspended resource and step, without re-eliciting settled content.
     - **Persona convened (fallback; suspended state not in this conversation)** → *"Facilitator ⚡ closing. Refined content is ready. [If gate fired: Close state: [state].] Resume **[convener name] [icon]** with the message \"continue `[initiative-name]` [mode/resource] from [artifact section in progress] with the [facilitator output] above.\""*

...
- [ ] Step 2: Inspect cross-artifact consistency and repository integration boundaries as needed. Resolve actual owner gaps; use experiments or explicit assumptions only for reversible engineering uncertainty.
- [ ] Step 3: Create or update the outcome section of `verification-matrix.md` from `assets/verification-matrix-template.md`. Link all applicable requirements to demonstrable evidence obligations, recording readiness and blockers. Preserve existing implementation/review evidence and other outcomes; invalidate only evidence affected by a changed source.
- [ ] Step 4: Register the matrix as live. Report sufficient readiness or the exact missing decision, and continue an already-authorized engagement. Do not create Slices or offer a separate planning gate merely because readiness passed.

...
- [ ] Step 2: Probe backward — review the existing `product-brief.md` for unresolved assumptions, queue items, or scope edges that still need promotion. Route user-owned gaps through `user-attention.md` and cross-artifact/source issues through `spec-patch-queue.md` before proceeding.
...
- [ ] Step 7: Register in context memory — open `[plan_folder]/<initiative-name>/_context.md`. Add `prd.md` to `## Live`. Move any superseded predecessor to `## Archived`.

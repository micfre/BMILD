...
- [ ] Step 4: Document — required when externally visible behaviour, operational runbooks, setup instructions, or user help changed. Otherwise record `Documentation impact: none`. If the fix leaves a remaining contract defect or requires an owner to promote a queued resolution, route it through `spec-patch-queue.md` or `user-attention.md` rather than a chat-only note.

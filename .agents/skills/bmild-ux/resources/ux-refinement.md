...
- **Preserve evidence and references in changed content.** A changed or newly added flow that translates a PRD journey keeps the protagonist, inline context, climax, failure path, and Firsthand/Illustrative label. Changed token references stay resolvable `{path.to.token}` names; changed components keep visual plus behavioral rules or a named inherited source; changed surfaces re-walk the applicable empty, cold-load, error, offline, and permission-denied states. A missing link surfaced by the change goes to the operator — never invent a screen, token, or component. Apply the same two-pass validation as UX-Design mode: mechanical coverage before judgment.
...
- [ ] Step 7: Consequence-check — verify changed sections, cross-section impacts, unaffected-outcome preservation, disposition clarity, and deferred-phase containment; verify changed flows preserve source journey evidence, surface closure still holds in both directions, and changed token/component/state references resolve or surface; run `completion-criteria.yaml` for all applicable sections.

...
- `slices.md` and `slice-<N>.md`: created by Sonia; consumed and updated by Alex; verified by Rahat and Zach; recut by Sonia when implementation reveals a planning problem. Readiness requires both PM artifacts except single-lane initiatives — exactly one design lane (`system-design.md` XOR `ux-design.md`) with no PM artifacts — which Sonia plans against the lane artifact's scope. Slice status lifecycle has named writers: Alex sets `status: ready-for-review` + `qa_status: ready_for_verification` (and `security_status: review_requested` when raising a security follow-up); Zach sets `security_status: findings_open` / `cleared` at review and handback closure (a clean review writes no artifact — `cleared` on the Slice is the closure record); Rahat sets `qa_status: verified | failed | blocked`, drives `status: done` when security is terminal, and moves the Slice to `## Archived` only at `done`.
...
- **Scribe path** (each standard persona's `references/scribe-path.md`; `docs/scribe-path.md` is a maintainer sync note only): a presiding persona may transcribe a *settled* fact into another owner's artifact in-context under the shared Scribe-Eligibility gate. This is **scribe ≠ author** — it transcribes settled facts; it never authors decisions (genuinely open items route). **Canonical-tier artifacts** (`context-map.md`, `[plan_folder]/adr/`, project-root `DESIGN.md`) are a hard fence and always route. Attribution is passive provenance (`applied_by_scribe`): traceability, not deferred audit — no required later action by the voiced owner. ≥2-owner cascades still route to Course-Correction. This is the second canonical consumer of each persona's `SOUL.md` (after roundtable attendee-voice loading), per `plans/adr/0003-persona-soul-canonical-voice.md`.
- **Promotion protocol** (each advanced facilitator's `references/promotion-protocol.md`): after a ratified durable-contract decision that leaves source artifacts stale, Roundtable / Elicit / Brainstorming ask once for promote authority, then apply only scribe-eligible inventory lines as presiding scribe (or persist a durable handoff/CC backlog). Authorization does not override Scribe-Eligibility, canonical-tier, high-stakes surfaces, or ≥2-owner Course-Correction. Close states: `ratified_and_promoted`, `ratified_and_routed`, `ratified_pending_authorization`, `ratified_with_documentation_deferred`. Identity guarded by `tests/scribe-path-contract.sh` and `tests/promotion-protocol-contract.sh`.
- **Consult path** (each standard persona's `references/consult-path.md`; 7 identical copies guarded by `tests/consult-path-contract.sh`): the middle tier of the resolution ladder — scribe (settled facts) → consult (open or settled-high-stakes, single-owner, non-canonical-tier) → route (`handoff.md` / Course-Correction). When `consult = 1|2` in `.bmild.toml`, the presiding persona dispatches the owning persona's harness-native consult subagent in-session; the subagent runs at that persona's pinned (model, reasoning-depth) tier (Frontier for Faisal/Katrina/Lance/Sonia, inherit for Alex/Rahat/Zach), authors the decision into its own artifact with `authored_by_consult` provenance, and returns a bounded decision record. Consult subagents are leaf nodes (no further dispatch; harness-enforced where supported); chain budget per gap is one owner consult plus at most one Sonia recut consult. Canonical-tier artifacts and ≥2-owner cascades always route; when consult is off or the harness lacks subagent dispatch, behaviour degrades byte-identically to the pre-consult workflow. Each persona also ships a thin `agents/consult.md` definition; `scripts/generate-consult-agents.sh` generates the per-harness packages (Claude Code `.claude/agents/`, Opencode `.opencode/agent/`, Codex `config.toml` fragment) and `scripts/build-releases.sh` bundles them under `harness/` in release tarballs.

...
- Added the **Close-the-Loop Gap Resolution** ladder across all seven standard personas and every mode: simplified scribe → capability-gated guest voice → owner consult → durable handoff → user-approved Course-Correction. The active persona now re-reads changed contracts, absorbs mechanical fallout, and resumes its suspended step. Same-owner consequences batch into one causal episode; independent owners resolve separately; Course-Correction is reserved for coupled scope/sequencing/proof changes. In-session resolutions record `applied_by_scribe`, `authored_by_guest`, or `authored_by_consult` beside the authoritative edit and never manufacture closed handoffs for audit history. Seven byte-identical `references/gap-resolution.md` copies replace the former scribe/consult references and are guarded by `tests/gap-resolution-contract.sh`.

- Added four regression guards for the runtime-fix round: `tests/state-lifecycle-contract.sh` (slice/QA/security lifecycle writers and the single done-gated archive move), `tests/pre-exit-offer-contract.sh` (same-turn continuation rule and bmild-elicit shortlist validity across the eight design-tier closes), `tests/facilitator-close-contract.sh` (guarded in-turn resume + copy-ready fallback parity across the three advanced facilitators), and `tests/parallel-budget-contract.sh` (concurrent estimator stress: plain exec, mixed-args, and shared-`$$` sourced invocation).

- Added **Project Bearing** to Faisal: a project-level direction mode for “what next?” questions. It reconciles live initiative state rather than trusting a stale rollup, presents a small set of load-bearing directions with evidence and overturn conditions, and makes one user-ratified recommendation. A selected bearing may be recorded in `rollup.md` `## Current Bearing`; it is advisory continuity, never an execution gate. The final, declinable offer can continue directly into existing Brief/PRD work or a user-authorized Sonia Course-Correction, while new initiatives reuse the existing slug-confirmation and Write-Product-Brief path. Guarded by `tests/project-bearing-contract.sh`.

- Added the **Ratification→Promotion** protocol for advanced facilitators: after a ratified durable-contract decision, Roundtable / Elicit / Brainstorming inventory mechanical, independent owner, coupled Course-Correction, and deferred delivery consequences. Authorized mechanical writes use simplified scribe without a persona-voice load; owner judgments return through separate ladder episodes; coupled fallout offers Course-Correction once and waits for consent. Close states remain `ratified_and_promoted`, `ratified_and_routed`, `ratified_pending_authorization`, and `ratified_with_documentation_deferred`.

- Added generated leaf consult packages for the three first-class harnesses only: current Claude Code frontmatter with model/effort and nested-delegation tools excluded; schema-valid Codex named roles with runtime model/reasoning overrides; and OpenCode inheritance with `task: deny` and no model/variant override.
...
- Replaced `consult`, `consult_model`, and `consult_effort` with `gap_resolution` and harness-specific `[intelligence.claude_code.<tier>]` / `[intelligence.codex.<tier>]` blocks. Legacy keys are a hard error with migration guidance and are never mapped. Design/planning use release-pinned highest-capability defaults when absent; implementation/reviewer inherit. Invalid explicit pairs never retry or substitute. OpenCode intentionally inherits the harness-wide model and ignores BMILD tier overrides.

- Made Slice/QA/security lifecycle state trustworthy end-to-end. `security_status` now has writers: Zach sets `findings_open` when a slice review produces an artifact and `cleared` on a clean review (no artifact — the Slice field is the closure record) and at Sec-Handback closure; Alex sets `security_status: review_requested` when raising a security follow-up and `qa_status: ready_for_verification` at Slice completion; Rahat sets `status: done` when verification passes with security terminal and moves the Slice to `## Archived` only then — archiving at `ready-for-review` is gone, so `## Archived` is terminal state (done or superseded), never mid-flight ambiguity. Verification now reconciles matrix items whose `Handoff reference` reached closed/applied (re-run the named proof, update with evidence), and the matrix template's `## Governance Checks` carries the closure rule: a stage is Open until its linked handoffs are closed AND its named proof has run. Dev Mode 1a infers the single active Slice from `slices.md` status instead of registry liveness. Guarded by `tests/state-lifecycle-contract.sh`.

- Made the sh slice-budget estimator safe for concurrent runs: temp files are `mktemp`-created (noclobber PID+counter fallback) instead of PID-suffixed names that collapse when two runs share `$$` (sourced or POSIX-subshell invocation semantics — the failure mode when an agent fans out subagents for budget estimation). TSV output contract unchanged (golden 39/39; `tests/parallel-budget-contract.sh` fails on the old script and passes on the new one). The PowerShell estimator holds records in memory and needed no change.

- Added a single-lane readiness exemption to Sonia: an initiative with exactly one design lane (`system-design.md` XOR `ux-design.md`) and no PM artifacts passes readiness and decomposes against the lane artifact's scope instead of blocking on a missing brief/PRD that pure architecture or pure UX work never needed. A brief without a PRD, or both lanes present without PM artifacts, still blocks. Applied in `readiness-verification.md`, `full-initiative-planning.md`, and `phase-scoped-planning.md`.

- Removed the double stop at the design-tier close and made the pre-exit offer intelligent: all eight PM/UX/Arch authoring and refinement resources now continue directly to Write/update in the same turn on any decline or proceed signal (no second confirmation), and the offer names 1–2 session-appropriate bmild-elicit methods from an embedded per-artifact shortlist (brief: Challenge from Critical Perspective, Shark Tank Pitch; PRD: Pre-mortem Analysis, Challenge from Critical Perspective; UX: User Persona Focus Group, Challenge from Critical Perspective; architecture: Architecture Decision Records, Failure Mode Analysis). Acceptance swaps to `bmild-elicit` with the method pre-selected — the elicit skill loads only then, never optimistically. Shortlist names are guarded against `methods.yaml` by `tests/pre-exit-offer-contract.sh`.

- Replaced the Roundtable debate step's BMAD-style `[C]/[S]/[E]` menu with a semantic end-of-round fork ("Another round on [the open tension], move to synthesis, or close the session — your call") and natural-language step routing; `step-03-synthesise.md` already used natural language.

- Made facilitator return seamless: when a persona convened the session and its suspended state is present in the conversation, Roundtable / Elicit / Brainstorming now resume the convener in-turn per its Same-Session Resumption contract (no Opening Stance re-emit, resume the suspended resource and step with the facilitator output as input) instead of handing the user a copy-paste invocation; the copy-ready invocation remains the fallback for fresh-window invocations. The anti-promise rule is reframed, not removed: never promise a resume — perform it or route it. Evolves the Slice-2 `enhance-interactivity` close shape; guarded by `tests/facilitator-close-contract.sh`.

- Closed the QA→Alex persistence gap in Verification mode: RCAs are no longer authored mid-verification (mandatory switch to Spec-Fix/Direct-Fix so Fix Election, RCA persistence, and the Alex handoff path apply), any Alex-actionable verification outcome is persisted in the Slice's `## QA / Security Follow-up` before close, and the close carries a verbatim Alex invocation so the assignment survives a fresh window. QA Spec-Fix now records declined-election open items in the same Follow-up section.

- Optimised the seven standard-persona `SOUL.md` files for weaker-LLM voice fidelity and richer cross-turn variety. Renamed the closing heading `## My perspective in one line` → `## My center of gravity` and reframed its content from quoted dialogue into descriptive stance prose. De-templated persona tensions and irritations, expanded formative influences, and synchronized the heading floor plus roundtable attendee-voice consumers. Simplified scribing no longer loads a `SOUL.md`; capability-gated guest voice and owner consult remain the only gap-resolution consumers of another persona's voice.
